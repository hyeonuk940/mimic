...
def run_xgboost(file_path, exclude_cols, target_col, use_grid_search=False, is_save=False, save_path='xgb_model.pkl'):
    """
    Args:
    ...
    # 숫자 데이터 확인 (XGBoost는 문자열을 직접 처리하지 못함)
    ...
    # ==================================================
    # [1] 학습 모드 선택 (단일 실행 vs 그리드 서치)
    # ==================================================
    model = None

    if use_grid_search:
        print("\n⚡ [모드: 그리드 서치 ON] XGBoost 최적의 파라미터 탐색 중...")
    ...
    print("\n" + "="*40)
    print(f"🏆 XGBoost 정확도 (Accuracy): {accuracy_score(y_test, y_pred):.4f}")
    print("="*40)
...
# 2. 기능 스위치
is_grid_search_on = True  # True: 최적화(느림), False: 기본(빠름)

# 3. 저장 설정
is_save_model = False        # True: 파일로 저장함, False: 저장 안 함
save_file_name = 'XGBoost_model_1.pkl' # 저장할 파일 이름
